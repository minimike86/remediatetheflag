...
public class GatewayHelper {
	...
	public ExerciseSelfCheckResult getSelfcheckResult(ExerciseInstance instance){
		...
		try {
			...
			for (Map.Entry<String, JsonElement> entry: entries) {
				if(ExerciseUtils.isInScope(entry.getKey(), instance.getAvailableExercise().getFlags())) {
	...
	public List<ExerciseResult> getResultStatus(ExerciseInstance instance, Challenge exerciseChallenge) {
		String ip = "";
		Integer httpPort = -1;
		...
		try {
			...
			for (Map.Entry<String, JsonElement> entry: entries) {
				if(ExerciseUtils.isInScope(entry.getKey(), instance.getAvailableExercise().getFlags())) {
					ExerciseResult r = ExerciseUtils.getExerciseResult(instance, exerciseChallenge, entry);
					if(null!=r)
						sList.add(r);
				}
			}
