...
public class PollReservationUpdateAction extends IAction {
	...
	public void doAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(null!=reservation.getEcs()) {
			...
			if(!updatedReservation.getFulfilled()) {
				...
				Challenge c = null;
				if(null!=reservation.getChallengeId()) {
					c = hpc.getChallengeWithDetailsForUser(reservation.getChallengeId(),user.getIdUser());
					ei.setType(ExerciseInstanceType.CHALLENGE);
					ei.setChallengeId(reservation.getChallengeId());
					ei.setScoring(c.getScoring());
				}
				else {
					ei.setType(ExerciseInstanceType.TRAINING);
					ei.setChallengeId(null);
					ei.setScoring(ExerciseScoringMode.MANUAL_REVIEW);
				}				
				...
				if(null==idExercise) {
					logger.error("User "+user.getIdUser()+" could not create exercise instance for : "+reservation.getEcs().getTaskArn()+" in gateway: "+gw.getName());
				}
				if(ei.getType().equals(ExerciseInstanceType.CHALLENGE)) {
					...
					if(runFlags != 0 && null!=c.getExercises() && !c.getExercises().isEmpty() && null!=c.getUsers() && !c.getUsers().isEmpty() && null!=c.getRunExercises() && !c.getRunExercises().isEmpty())
