...
public class GetGuacTokenForExerciseAction extends IAction{
	...
	public void doAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(exercise.getGuac()!=null){
			token = guacHelper.getFreshToken( exercise.getGuac().getGateway(), exercise.getGuac().getUsername(), exercise.getGuac().getPassword());
		}
		else{
		...
		if(null!=token){
			MessageGenerator.sendTokenMessage(exercise.getIdExerciseInstance(),exercise.getGuac().getGateway().getFqdn(), exercise.getGuac().getUsername(), token, 0, response);
		}
		else{
