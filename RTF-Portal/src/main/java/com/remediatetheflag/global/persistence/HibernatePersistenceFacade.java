...
public class HibernatePersistenceFacade {
	...
	public List<AvailableExercise> getAllAvailableExercises(){
		HibernateSessionTransactionWrapper hb = openSessionTransaction();
		try {
			List<AvailableExercise> exercises = hb.localSession.createQuery("from AvailableExercise a where a.status != :superseded")
		...
		try {
			...
			for(AvailableExercisesForOrganization e : exercises) {
				...
				for(AvailableExercise ex : e.getExercises()){
					if(!ex.getStatus().equals(AvailableExerciseStatus.SUPERSEDED))
						tmpEx.add(ex);
				}
	...
	public List<AvailableExercise> getAllAvailableTrainingExercises(Organization organization){
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if(ex.getStatus().equals(AvailableExerciseStatus.SUPERSEDED) || ex.getExerciseType().equals(AvailableExerciseType.CHALLENGE)) {
					iter.remove();              
				}
			}
			return exercises;
	...
	public Boolean deleteGateway(Integer id) {
		...
		try {
			...
			List<RTFECSTaskDefinition> ecsTasks = hb.localSession.createQuery("from RTFECSTaskDefinition where region = :reg")
					...
					.getResultList();
			Boolean anotherAvailableForRegion = false;
			if(ecsTasks.size()>0) {
				List<RTFGateway> gateways = getAllGateways();
				for(RTFGateway g : gateways) {
					if(!g.getId().equals(gw.getId()) && g.getRegion().equals(gw.getRegion())) {
						anotherAvailableForRegion = true;
					}
				}
			}
			if(!anotherAvailableForRegion)
				return false;
	...
	public Boolean removeAvailableExercise(Integer idExercise) {
		...
		try {
			...
			ex.setStatus(AvailableExerciseStatus.SUPERSEDED);
	...
	@SuppressWarnings("unchecked")
	public List<Challenge> getChallengesForUserExercise(Integer idExercise, Integer idUser) {
		HibernateSessionTransactionWrapper hb = openSessionTransaction();
		List<Challenge> cList;
		try{
			cList = hb.localSession.createQuery("select c from Challenge as c inner join c.users as usrs inner join c.exercises as ex "
					+ "where usrs.idUser = :usrId "
					+ "and ex.id = :idExercise "
					+ "and c.status != :status")
					.setParameter( "usrId", idUser )
					.setParameter( "idExercise", idExercise )
		...
		try{
			...
			for(Challenge ei : cList){
				...
				for(ExerciseInstance i : ei.getRunExercises()) {
					if(i.getAvailableExercise().getId().equals(idExercise)) {
						alreadyRun = true;
					}
				}
			}
			closeSessionTransaction(hb);
			if(inChallenge && alreadyRun)
				return false;
			if(inChallenge)
				return true;
			return false;
	...
	public Challenge getChallengeFromId(Integer challengeId) {
		...
		try{
			...
			for(ExerciseInstance i : ei.getRunExercises()){
				Hibernate.initialize(i.getAvailableExercise());
	...
	public Challenge getChallengeFromName(String name) {
		...
		Challenge ei;
		try{
			ei = (Challenge) hb.localSession.createQuery("from Challenge where name = :name ")
	...
	public List<ExerciseInstance> getManagegementPendingExerciseInstances(Set<Organization> organizations){
		...
		try {
			List<ExerciseInstance> ei = hb.localSession.createQuery("from ExerciseInstance instance "
					+ "where instance.organization.id in (:names) and (instance.status = :pending or instance.status = :stopping)")
					.setParameter("pending", ExerciseStatus.STOPPED)
					.setParameter("stopping", ExerciseStatus.STOPPING)
					.setParameterList("names", ids)
					.getResultList();
			for(ExerciseInstance ex : ei){
				Hibernate.initialize(ex.getUser());
	...
	public List<ExerciseInstance> getManagementReviewedExerciseInstances(List<User> users) {
		...
		try{
			List<ExerciseInstance> ei = hb.localSession.createQuery("from ExerciseInstance ei "
					+ "where ei.user.idUser in (:names) and (ei.status =:reviewed or ei.status =:cancelled or ei.status =:autoreviewed or ei.status =:autoreviewed_modified)")
					.setParameter( "reviewed", ExerciseStatus.REVIEWED)
					.setParameter( "autoreviewed", ExerciseStatus.AUTOREVIEWED)
					.setParameter( "autoreviewed_modified", ExerciseStatus.REVIEWED_MODIFIED)
					.setParameter( "cancelled", ExerciseStatus.CANCELLED)
					.setParameterList("names", ids)
					.getResultList();
			for(ExerciseInstance ex : ei){
	...
	public List<ExerciseInstance> getManagementReviewedExerciseInstances(Set<Organization> organizations) {
		...
		try{
			List<ExerciseInstance> ei = hb.localSession.createQuery("from ExerciseInstance ei "
					+ "where (ei.status =:reviewed or ei.status =:autoreviewed or ei.status =:autoreviewed_modified or ei.status =:cancelled) and ei.organization.id in (:names)")
					.setParameter( "reviewed", ExerciseStatus.REVIEWED)
					.setParameter( "autoreviewed", ExerciseStatus.AUTOREVIEWED)
					.setParameter( "autoreviewed_modified", ExerciseStatus.REVIEWED_MODIFIED)
					.setParameter( "cancelled", ExerciseStatus.CANCELLED)
	...
	public ExerciseInstance getManagementCompletedExerciseInstance(Integer idExerciseInstance, Set<Organization> organizations) {
		...
		try{
			...
			Hibernate.initialize(ei.getResultFile());
			Hibernate.initialize(ei.getUser());
			Hibernate.initialize(ei.getExercise().getFlags());
			for(ExerciseResult er : ei.getResults()){
				Hibernate.initialize(er);
			}
			closeSessionTransaction(hb);
	...
	@SuppressWarnings("unchecked")
	public List<RTFECSTaskDefinitionForExerciseInRegion> getAllTaskDefinitionsForExercise(Integer idExercise) {
		HibernateSessionTransactionWrapper hb = openSessionTransaction();
		try{
	...
	@SuppressWarnings("unchecked")
	public  void updateAllTaskDefinitionsForExercise(Integer oldExercise, Integer newExercise) {
		HibernateSessionTransactionWrapper hb = openSessionTransaction();
		AvailableExercise availableExercise = getAvailableExerciseComplete(newExercise);
		try{
			List<RTFECSTaskDefinitionForExerciseInRegion> list = (List<RTFECSTaskDefinitionForExerciseInRegion>) hb.localSession.createQuery(
					"from ECSTaskDefinitionForExerciseInRegion where exerciseId = :exId")
					.setParameter( "exId", oldExercise )
					.getResultList();
			for(RTFECSTaskDefinitionForExerciseInRegion ecsTask: list) {
				ecsTask.setExercise(availableExercise);
				hb.localSession.update(ecsTask);
			}
			closeSessionTransaction(hb);
		} catch(Exception e){
			closeSessionTransaction(hb);
	...
	public RTFECSTaskDefinition getTaskDefinitionFromUUID(String uuid, Regions region) {
		HibernateSessionTransactionWrapper hb = openSessionTransaction();
		try{
			RTFECSTaskDefinitionForExerciseInRegion td = (RTFECSTaskDefinitionForExerciseInRegion) hb.localSession.createQuery("select t from ECSTaskDefinitionForExerciseInRegion t join t.exercise ex where "
